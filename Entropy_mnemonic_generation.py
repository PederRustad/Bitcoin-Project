...
def entropy_to_mnemonic24(entropy):
    ...
    print("indexes = ", indexes, "len(indexes) =", len(indexes))
    indexes_bits = ''.join(format(index, '011b') for index in indexes) #make bit version of key
    #indexes_byte = bytes(int(format(index, '08b'), 2) for index in indexes) #make byte version of key

    #print ("index post checksum", indexes, "entropy digest", sha256(entropy).digest()[0])
    print(len(indexes_bits), indexes_bits)
